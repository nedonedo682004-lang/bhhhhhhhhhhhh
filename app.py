...
PARALLEL_WORKERS = 100  # عدد الطلبات اللي تشتغل في نفس الوقت
...
def run_card_check_parallel(cc, mes, ano, cvv, site_url, variant_id=None, proxy_str=None):
    """
    نفس وظيفة process_card_async لكن بتشتغل في thread منفصل
    (دا مش بيعدل على الكود الأصلي، دا بيدي طريقة جديدة لتشغيله)
    """
    # نعمل event loop جديد لكل thread (لأن الكود الأصلي محتاج كده)
...
# ═══════════════════════════════════════════════════════════════════
# إضافة معالجة متوازية (Parallel Processing) - بدون تعديل الكود الأصلي
# ═══════════════════════════════════════════════════════════════════
...
def shopify_checker_parallel():
    ...
    try:
        ...
        response_data = {
            ...
            "Status": success,
            "cc": cc_string,
            "parallel_mode": True,  # علامة إن الطريق استخدم النسخة المتوازية
            "active_requests": _active_requests  # عداد الطلبات النشطة
        }
...
def parallel_stats():
    """
    عرض إحصائيات المعالجة المتوازية
    """
    return jsonify({
...
# ملاحظة: الكود الأصلي /shopify لسه شغال زي ما هو، ماتغيرش خالص
